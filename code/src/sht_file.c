...
#define MAX_DEPTH (8*sizeof(uint)-8) // Maximum global depth of the hash table
...
HT_ErrorCode SHT_PrintAllEntries(int sindexDesc, char *index_key ) {
  ...
  CALL_BF(BF_UnpinBlock(Stat));
  BF_Block_Destroy(&Stat);
  BF_Block_Destroy(&targetBlock);
  BF_Block_Destroy(&mainTargetBlock);
